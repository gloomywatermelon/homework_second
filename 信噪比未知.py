...
# 维纳滤波函数
def wiener_filter(image, noise_power_spectrum, snr=None):
    # 计算维纳滤波器的频域表达式
    if snr is None:
        # 信噪比未知情况下，使用最大似然估计计算信噪比
        snr = np.mean(np.abs(image) ** 2) / np.mean(noise_power_spectrum)

    wiener_filter = 1 / (1 + snr / noise_power_spectrum)
...
# 进行图像复原
restored_image = wiener_filter(image, noise_power_spectrum)
